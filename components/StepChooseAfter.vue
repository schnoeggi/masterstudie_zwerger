<template>
  <section class="text-center mt-15 px-3">
    <p class="text-body-1">
      Sie haben nun den Algorithmus kennengelernt. Bitte geben Sie anhand der Schieberegler an, was Sie zukünftig lieber möchten in Bezug auf
      folgende Aussagen.
...
import questions from "../assets/afterchoose.json"
...
export default {
  ...
  methods: {
    ...
    next() {
      this.addToData(Object.values(this.questions).map(q => ({[`After: ${q.question}`]: q.selected})))
      this.$emit('next');
    },
    color(value){
      const colors = {
